// ✅ INICIO CORRECCIÓN: Script de navegación para el Dashboard
function irATab(tabName, panelId) {
    // Activa la pestaña correcta usando el atributo data-tab
...
// Sistema de debounce para optimizar búsquedas
let debounceTimers = {};
...
// Versiones con debounce de las funciones de actualización
...
// ===== SISTEMA DE PAGINACIÓN PARA TABLAS DE ESTADÍSTICAS =====
class TablePagination {
    constructor(tableId, itemsPerPage = 20) {
        ...
        render() {
        // Implementado específicamente para cada tabla
    }
}
// Instancias globales
...
async function cargarTodosLosDatos() {
    try {
        ...
        // Cargar torneo activo primero (crítico)
        await cargarTorneoActivo();

        // Cargar datos principales en paralelo (optimización)
        await Promise.all([
            cargarEquipos(),
            cargarJugadores(),
            cargarPartidos()
        ...
        // Cargar estadísticas después (no bloqueante)
        ...
        console.log(`✅ Sistema cargado en ${loadTime} segundos`);
...
async function agregarJugador() {
    ...
    try {
        ...
        console.log('✅ Jugador creado:', resultado);
        ...
        if (nombreInput) nombreInput.style.borderColor = 'rgba(255, 215, 0, 0.3)';
...
// Validación en tiempo real para el formulario de jugadores
function configurarValidacionJugador() {
    ...
    nombreInput.addEventListener('input', debounce(function() {
        ...
        if (valor.length === 0) {
            ...
            errorDiv.style.display = 'none';
            nombreInput.style.borderColor = 'rgba(255, 215, 0, 0.3)';
            btnAgregar.disabled = false;
        } else if (valor.length < 3) {
            errorDiv.textContent = '⚠️ El nombre debe tener al menos 3 caracteres';
            errorDiv.style.display = 'block';
            errorDiv.style.color = '#ffa500';
            nombreInput.style.borderColor = '#ffa500';
            btnAgregar.disabled = true;
        } else if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(valor)) {
            errorDiv.textContent = '⚠️ Solo se permiten letras y espacios';
            errorDiv.style.display = 'block';
            errorDiv.style.color = '#ffa500';
            nombreInput.style.borderColor = '#ffa500';
            btnAgregar.disabled = true;
        } else {
            errorDiv.textContent = '✓ Nombre válido';
            errorDiv.style.display = 'block';
...
async function cargarEquipos() {
    try {
        ...
        console.log('📦 Datos recibidos de /api/equipos:', data);
        
        // Asegurar que sea un array
        equipos = Array.isArray(data) ? data : [];
        
        console.log(`✅ Equipos cargados correctamente: ${equipos.length} registros`);
        ...
        if (errorContainer) {
            errorContainer.textContent = 'Error al cargar equipos. Intenta recargar la página.';
...
async function cargarPartidos() {
    try {
        ...
        const data = await response.json();
        console.log('📦 Datos recibidos de /api/partidos:', data);
                
        // CORRECCIÓN: Manejo robusto de diferentes formatos de respuesta
        if (!data) {
            partidos = [];
            ...
            console.warn('⚠️ Formato inesperado de partidos:', data);
            partidos = [];
        }
        
        console.log(`✅ Partidos cargados correctamente: ${partidos.length} registros`);
        ...
        if (errorContainer) {
            errorContainer.textContent = 'Error al cargar partidos. Intenta recargar la página.';
            errorContainer.style.display = 'block';
        }
    }
}
...
async function cargarJugadores() {
    try {
        ...
        const data = await response.json();
        console.log('📦 Datos recibidos de /api/jugadores:', data);
        ...
        if (!data) {
            ...
            console.warn('⚠️ Formato inesperado de jugadores:', data);
            jugadores = [];
        }
        
        console.log(`✅ Jugadores cargados correctamente: ${jugadores.length} registros`);
        ...
        if (errorContainer) {
            errorContainer.textContent = 'Error al cargar jugadores. Intenta recargar la página.';
            errorContainer.style.display = 'block';
        }
    }
}
...
function actualizarTablaPartidos() {
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="7" class="empty-state" style="padding: 40px 20px;">
                    ...
                    <h3 style="color: #ffd700; margin-bottom: 10px;">No se encontraron partidos</h3>
    ...
    tbody.innerHTML = partidosFiltrados.map(partido => {
        ...
        const estadoBadge = 
            partido.estado === 'programado' ? '<span style="color: #ffd700;">📅 Programado</span>' :
            partido.estado === 'finalizado' ? '<span style="color: #4CAF50;">✅ Finalizado</span>' :
            partido.estado === 'cancelado' ? '<span style="color: #dc143c;">❌ Cancelado</span>' :
            (partido.carreras_local !== null ? '<span style="color: #4CAF50;">✅ Finalizado</span>' : '---');
...
function configurarFiltrosEnTiempoReal() {
    ...
    if (buscadorJugadores) {
        ...
        console.log('✅ Buscador de jugadores configurado con debounce (300ms)');
    }
    ...
    if (buscadorEquipos) {
        ...
        console.log('✅ Buscador de equipos configurado con debounce (300ms)');
    }
...
function generarAlertas() {
    ...
    if (alertas.length === 0) {
        ...
        // ✅ INICIO CORRECCIÓN: Añadir IDs a los botones
        alertasContainer.innerHTML = alertas.map(alerta => {
            ...
            if (alerta.accion) {
                ...
                } else if (alerta.accion === 'Registrar Estadísticas') {
    ...
    // ✅ CORRECCIÓN: Asignar eventos a los botones recién creados
...
async function crearTorneo() {
    ...
    try {
        ...
        toast.error('Error de conexión al crear torneo.');
    }
}
...
async function programarPartido() {
    ...
    console.log('📤 Programando partido:', body);
    try {
...
async function registrarEstadisticas() {
    ...
    // ✅ CORRECCIÓN: Verificar que haya un torneo activo
    if (!torneoActivo || !torneoActivo.id) {
        toast.error('No hay un torneo activo. Activa uno para registrar estadísticas.');
        return;
    }
    ...
    const data = {
        ...
        temporada: torneoActivo.id // ✅ CORRECCIÓN: Añadir ID del torneo activo
    };
    ...
    try {
        ...
        if (res.ok) {
            ...
            toast.error('Error al registrar estadísticas.');
        }
    } catch(e) {
        toast.error('Error de red.');
    }
...
async function eliminarEquipo(id) {
    ...
    const confirmacion = confirm(
        `⚠️ ELIMINAR EQUIPO\n\n` +
        `¿Estás seguro de eliminar al equipo "${nombreEquipo}"?\n\n` +
...
async function editarPartido(id) {
    ...
    if (partido.estado === 'programado' || (partido.carreras_local === null && partido.estado !== 'finalizado')) {
        ...
        // Cambia el botón para actualizar
        const btn = document.querySelector('#partidos .card:first-child .btn-primary');
        btn.textContent = '✅ Actualizar Partido Programado';
        btn.onclick = () => actualizarPartido(id);

        toast.info('Editando partido programado. Puede añadir los resultados en el formulario de abajo.');
        ...
        document.getElementById('btn-registrar-partido').textContent = '✅ Actualizar Resultado';
...
async function actualizarPartido(id) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            btnProg.textContent = '📅 Programar Partido';
...
async function registrarEstadisticasPitcheo() {
    ...
    try {
        ...
        if (res.ok) {
            toast.success('Estadísticas de pitcheo registradas.');
            await cargarEstadisticas();
...
async function registrarEstadisticasDefensivas() {
    ...
    try {
        ...
        if (res.ok) {
            toast.success('Estadísticas defensivas registradas.');
            await cargarEstadisticas();
...
async function cargarTablaEditarBateo() {
    ...
    if (!document.getElementById('controls-bateo')) {
        ...
        controls.innerHTML = `
            ...
            <div class="results-info">
                ...
                <button class="btn-secondary" onclick="limpiarFiltrosBateo()" style="background: #ff8c00; padding: 8px 16px;">🗑️ Limpiar</button>
            </div>
        `;
    ...
    try {
        ...
        }).filter(Boolean); // Filtrar nulos si un jugador no se encontró
...
function renderPagination(type, pagination) {
    ...
    `;
    // Mostrar páginas (máximo 5 botones)
...
async function cargarTablaEditarPitcheo() {
    ...
    if (!document.getElementById('controls-pitcheo')) {
        ...
        controls.innerHTML = `
            ...
            <div class="results-info">
                ...
                <button class="btn-secondary" onclick="limpiarFiltrosPitcheo()" style="background: #ff8c00; padding: 8px 16px;">🗑️ Limpiar</button>
            </div>
        `;
        table.parentElement.insertBefore(controls, table);
    ...
    try {
        tbody.innerHTML = '<tr><td colspan="8" class="loading">Cargando estadísticas...</td></tr>';
    ...
    if (!document.getElementById('controls-defensa')) {
        ...
        controls.innerHTML = `
            ...
            <div class="results-info">
                ...
                <button class="btn-secondary" onclick="limpiarFiltrosDefensa()" style="background: #ff8c00; padding: 8px 16px;">🗑️ Limpiar</button>
            </div>
        `;
        table.parentElement.insertBefore(controls, table);
...
// ==================================================================
// ✅ INICIO CORRECCIÓN BUG #1: DUPLICACIÓN DE JUGADORES
// Se reemplaza la función original por esta versión más robusta.
// Esta función envía los datos de las tablas de edición al backend.
// La lógica del backend (en server.js) debe estar corregida para
...
async function actualizarEstadistica(tipo, jugadorId) {
    ...
    const temporada = torneoActivo ? torneoActivo.id : null; // Asumiendo que se usa el torneo activo.

    if (!temporada) {
        toast.error('No hay un torneo activo seleccionado. No se pueden guardar estadísticas.');
        return;
    }
    ...
    try {
        ...
        switch (tipo) {
            case 'bateo':
                ...
                // Nota: Se envían solo los campos editables en esta tabla.
                // El backend debe manejar los demás campos (runs, walks, etc.)
                break;
        ...
        console.log(`📤 [CORRECCIÓN BUG #1] Enviando actualización (${tipo}) para jugador ${jugadorId}:`, payload);
        
        // Usamos POST para la lógica de UPSERT (INSERT ... ON CONFLICT) como se sugiere
        // en la solución. Si el backend usa PUT para reemplazo, se puede cambiar el método.
        const response = await fetch(endpoint, {
            method: 'POST', // Usamos POST para que coincida con la lógica de UPSERT.
            headers: {'Content-Type': 'application/json'},
        ...
        if (response.ok) {
            ...
            await cargarEstadisticas(); // Recargar todas las estadísticas para consistencia
            actualizarLideres(); // Actualizar los líderes que pudieron cambiar
        } else {
            const error = await response.json();
        ...
        console.error(`❌ Error actualizando estadística de ${tipo}:`, error);
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="6" class="empty-state" style="padding: 40px 20px;">
                    <div style="font-size: 3rem; margin-bottom: 15px;">🏆</div>
                    <h3 style="color: #ffd700; margin-bottom: 10px;">No se encontraron equipos</h3>
                    <p style="color: #fff; opacity: 0.7; font-size: 0.9rem;">
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="5" class="empty-state" style="padding: 40px 20px;">
                    <div style="font-size: 3rem; margin-bottom: 15px;">👤</div>
                    <h3 style="color: #ffd700; margin-bottom: 10px;">No se encontraron jugadores</h3>
                    <p style="color: #fff; opacity: 0.7; font-size: 0.9rem;">
